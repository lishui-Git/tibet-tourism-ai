...
复核项（均对照 `docs/diagrams/数据处理流程图.md` §2 的实测结果）：
    ...
    3. 景点级字段：地址 / 开放时间 / 景点介绍 **3 个**按代表值规则
       （次数最多 → 并列取最长 → 再并列取首次出现）跨全量重算比对。
       **官方电话（第 4 个景点级字段）不在本脚本复核范围内**：它的口径含
       `spot.phone VARCHAR(64)` 的截断守卫，本脚本不重复实现该规则，只核对非空。
    4. 派生字段：content_length 与实际正文长度自洽、is_low_info 与 ≤10 字口径自洽、
       发布时间格式合法
    5. 关键口径：低信息量 10,228、重复正文 4,239 条／1,285 组、IP 未知 24,025、
       评分空值 37、正文空值 1、5 星 42,337、景点 837、IP 省份已填 34,578
    6. 分布与范围：发布时间范围、正文长度范围、评分分布（**仅打印，不参与成败判定**）

说明：源文件的完整性（大小与 SHA256）不在本脚本内校验——该比对在
`clean_dataset.py` 执行时人工记录，报告文件中也记有输入文件路径与行数。
